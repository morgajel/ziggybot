#!/usr/bin/perl
# The following code is the gong module

#TODO This is the gong module
#if you bang(!) the gong, ziggy gets everyone's attention
#<morgajel>!gong
#<ziggy> !bangs a gong next to mors, ogg, wonko
#<ziggy> !bangs a gong next to shaldannon, oldman, general
#<ziggy> !bangs a gong next to shabbs, dendrite, mrbojangles
#<ziggy> !bangs a gong next to myjaxon
#etc...
exit 1;
...
#
    ...
    #Common functions are located in the lib directory
		my $logger = get_logger("Ziggy::Gong");
    use Common qw( filter );


	my $logger = get_logger("Ziggy::Gong");

    $logger->debug("loading gong");  
...
# For example, we're catching a public server message (when someone speaks in a channel we're in)
    sub S_public {
        ...
        if ( $msg =~ /your mom/i       and       time > $ziggy_data->{'gong'}->{'nextup'} ){
            ...
            $logger->info("someone said your mom. interval:". $ziggy_config_ref->{'gong'}->{'interval'}." nextup:".$ziggy_data->{'gong'}->{'nextup'}." time:".time);
            ...
            $irc->_send_event( 
                        ...
                        $delay );
            $ziggy_data->{'gong'}->{'nextup'}=time+$ziggy_config_ref->{'gong'}->{'interval'};
...
sub get_saying{
        use vars qw ($ziggy_config_ref $ziggy_data);
		my $logger = get_logger("Ziggy::Gong");
        my $id=int rand(scalar (@{ $ziggy_config_ref->{'gong'}->{'saying'} } ));
        my $saying=$ziggy_config_ref->{'gong'}->{'saying'}[$id]->{'content'};
        if ($ziggy_config_ref->{'gong'}->{'intro_chance'} >= int(rand(100))){
            $logger->info("adding an intro...");
          my $intro_id= int(  rand(  scalar (  @{ $ziggy_config_ref->{'gong'}->{'intro'}  }  )   )   );
          $saying=$ziggy_config_ref->{'gong'}->{'intro'}[$intro_id]->{'content'} . $saying;
    ...
    sub S_msg {
        ...
        #yes it's ugly, and the second line makes it sane.
        my ($nick)= split /!/ , $who ;
        $logger->info("help for gong...");
		if( $msg =~/help/i ){
            if( $msg =~/help *gong/i ){
                $logger->info("help for gong...");
                &help($ziggy_config_ref,$nick,$irc);
            }elsif($msg =~/help *$/){
                $irc->_send_event( 'say' ,  $nick, "gong",1);
            }
            return PCI_EAT_NONE;
        }
        return PCI_EAT_NONE;
    ...
    sub help {
		...
		my $logger = get_logger("Ziggy::Gong");
        $irc->_send_event( 'say' ,  $target, "      gong:",1);
